...
public class RobotActions {
    ...
    public void move(int relativeX, int relativeY) {
        MoveMessage message = new MoveMessage(random.nextInt(), 0, hub.getPort());
    ...
    public void turn(int relativeOrientation) {
        TurnMessage message = new TurnMessage(random.nextInt(), 0, hub.getPort());
        message.setDegrees(new UnsignedInt(relativeOrientation));
        hub.send(message);
    }

    public void changeSize(int newSize) {
        ChangeSizeMessage message = new ChangeSizeMessage(random.nextInt(), 0, hub.getPort());
        message.setDiameter(new UnsignedInt(newSize));
        hub.send(message);
    }

    public void changeColor(Color newColor) {
        ChangeColorMessage message = new ChangeColorMessage(random.nextInt(), 0, hub.getPort());
        message.setColor(newColor);
        hub.send(message);
    }

    public void whoIsThere(UnsignedInt distance) {
        WhoIsThereMessage message = new WhoIsThereMessage(random.nextInt(), 0, hub.getPort());
    ...
    public void parameterReport(Integer integralPart, UnsignedInt realPart, UnsignedByte parameterId) {
        ParameterReportMessage message = new ParameterReportMessage(random.nextInt(), 0, hub.getPort());
